...
const databasePath = await mkdtemp(join(temporaryRoot, "ambiten-bonus-runtime-"));
const children = [];
let database;
...
async function main() {
  ...
  const uri = `mongodb://127.0.0.1:${mongoPort}/?replicaSet=tutorial12`;
  const mongo = launch(process.env.MONGOD_BINARY ?? "mongod", [
    "--bind_ip", "127.0.0.1", "--port", String(mongoPort),
    "--dbpath", databasePath, "--replSet", "tutorial12", "--quiet"
  ]);
  ...
  await database.db("admin").command({
    replSetInitiate: { _id: "tutorial12", members: [{ _id: 0, host: `127.0.0.1:${mongoPort}` }] }
  });
  ...
  const initialReadiness = await fetch(`${origin}/ready`);
  assert.equal(initialReadiness.status, 200);
  assert.deepEqual(await initialReadiness.json(), {
    ready: true,
    runtime: { initialized: true, shuttingDown: false, tenants: { registered: 2, connected: 0 } }
  });
  for (const header of ["x-db-name", "x-collection-name"]) {
    const rejected = await fetch(`${origin}/users`, {
      headers: { "x-tenant-id": "tenant-a", [header]: "arbitrary-client-scope" }
    });
    assert.equal(rejected.status, 400);
  }
  console.log("PASS: tenant-free health/readiness, lazy startup, and server-owned database/collection routing");
  ...
  assert.equal((await (await fetch(`${origin}/ready`)).json()).runtime.tenants.connected, 2);
  ...
  for (const [overrides, message] of [
    [{ MONGO_URI: "" }, /Missing required environment variable: MONGO_URI/],
    [{ DB_NAME: "  " }, /Missing required environment variable: DB_NAME/],
    [{ PORT: "0" }, /Invalid PORT/],
    [{ PORT: "65536" }, /Invalid PORT/],
    [{ PORT: "3.5" }, /Invalid PORT/],
    [{ PORT: "not-a-number" }, /Invalid PORT/]
  ]) {
    const invalid = await finishProcess(launch(process.execPath,
      ["--input-type=module", "-e", "await import('./dist/config/env.js')"], { ...env, ...overrides }));
    assert.equal(invalid.child.exitCode, 1, invalid.output);
    assert.match(invalid.output, message);
  }
  const portCollision = await finishProcess(launch(process.execPath, ["--import", "tsx", "dist/index.js"], env));
  assert.equal(portCollision.child.exitCode, 1, portCollision.output);
  assert.match(portCollision.output, /EADDRINUSE/);
  assert.match(portCollision.output, /Disconnected from MongoDB/);
  console.log("PASS: required configuration, valid TCP port range, and HTTP bind-failure cleanup");

  for (const scenario of ["readiness-and-idempotence", "startup-race", "cleanup-failure", "http-drain-SIGINT", "http-drain-SIGTERM"]) {
    const probe = await finishProcess(launch(process.execPath,
      ["--import", "tsx", "scripts/runtime-checks.mjs", scenario], { ...env, PORT: String(await freePort()) }));
    assert.equal(probe.child.exitCode, 0, probe.output);
    assert.ok(probe.output.includes(`PASS: runtime ${scenario}`), probe.output);
...
try {
  ...
  assert.ok(basename(databasePath).startsWith("ambiten-bonus-runtime-"));
