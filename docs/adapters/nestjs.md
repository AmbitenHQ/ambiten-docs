...
The NestJS adapter integrates Tenra through NestJS’s module and interceptor system rather than traditional middleware.

Instead of attaching runtime behavior directly to route handlers, the adapter establishes execution scope at the framework level so controllers, services, and providers execute inside a fully initialized `TenraContext`.

This allows Tenra to align naturally with NestJS’s dependency injection model, modular architecture, and request lifecycle.

## Integration model

The adapter is installed through `TenraNestAdapterModule.forRoot(...)`.
...
The Tenra adapter integrates through a global interceptor that executes before controller logic runs.

Internally, the adapter normalizes the incoming request into a runtime-aware request shape, invokes the adapter runtime, initializes TenraContext, and then allows controller execution to continue inside the active runtime boundary.
...
The runtime lifecycle follows the same architectural path as other Tenra adapters while adapting to NestJS’s execution model.
...
<SignalFlow 
   ...
   :items='["HTTP Request", "NestJS Interceptor", "Adapter Runtime", "TenraContext", "Controller", "Service", "MongoDB"]'
/>

NestJS continues managing routing and dependency injection while Tenra establishes runtime-aware execution boundaries around the request lifecycle.
...
Tenant identity can be resolved automatically from request metadata, `TenraContext` remains available throughout services and model execution, and middleware and instrumentation behave consistently across the request lifecycle.
...
Services can participate in transactions through the same runtime-aware transaction model used everywhere else in Tenra.
...
Because of this, Tenra integrates through:

```PlainText
Dynamic Module  → configuration
Global Interceptor → execution boundary
TenraContext → runtime state
...
Once execution enters the runtime boundary, models and middleware behave consistently across every supported Tenra environment.

## Summary

The NestJS adapter connects NestJS’s execution pipeline to Tenra’s runtime system.

It establishes runtime-aware execution through modules and interceptors, initializes `TenraContext` before controller execution begins, enables tenant-aware and transaction-aware workflows, and keeps controllers and services focused on application behavior instead of infrastructure coordination.
