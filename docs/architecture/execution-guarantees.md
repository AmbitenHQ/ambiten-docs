...
Execution guarantees define what Tenra ensures at runtime, regardless of application complexity.
...
This page is intentionally explicit. It clarifies both what Tenra guarantees and what it does not.

This page explains how Tenra guarantees execution behavior. If you are looking for how execution moves through the runtime, see [Runtime Execution Flow](/architecture/runtime-execution-flow).

<DocOverviewCards
  eyebrow="Runtime Contract"
  title="Tenra guarantees correctness inside a single execution boundary."
  ...
  :cards='[
    ...
    { "label": "Bounded", "title": "Tenra does not pretend to own distributed systems", "text": "Cross-service transactions, network reliability, external side effects, and global tenant consistency remain application or infrastructure concerns." }
  ]'
  :flow='[
    { "label": "Boundary", "title": "TenraContext" },
...
Tenra defines its guarantees so that runtime behavior is predictable, testable, and safe to compose upon.

## Guaranteed behavior

### Context propagation

Tenra guarantees that `TenraContext` is preserved across asynchronous execution within a single runtime boundary.
...
When a transaction is active, Tenra guarantees that all model operations within that boundary execute using the same MongoDB session.
...
Tenra guarantees that, when tenant context is present and correctly resolved, all model operations execute within that tenant’s scope.
...
Tenra guarantees that provider resolution within a runtime boundary remains stable for the lifetime of that execution context.
...
Tenra guarantees deterministic middleware execution.
...
Tenra guarantees that instrumented operations emit structured telemetry with consistent metadata.
...
A runtime boundary begins when `TenraContext.run()` or an adapter runtime initializes execution context and ends when execution leaves that scope.

Context state is not guaranteed outside the active boundary lifetime.

## Non-guaranteed behavior

Tenra is explicit about what it does not guarantee.

These boundaries are intentional.

### Cross-service transactions

Tenra does not guarantee transaction continuity across multiple services or processes.
...
Tenra does not guarantee network availability, MongoDB cluster health, or external service responsiveness.
...
Tenra does not enforce consistency across tenants.
...
If the external call fails or behaves unpredictably, Tenra cannot guarantee consistency between that system and MongoDB.

### Long-running execution safety

Tenra does not guarantee safety for:
...
Tenra’s guarantees are focused on execution correctness within a single runtime boundary.
...
At the same time, Tenra deliberately avoids overextending into areas that belong to:
...
```Plain text
Tenra guarantees correctness inside the runtime.
The system guarantees correctness across runtimes.
```

## Summary

Execution guarantees define the contract between Tenra and the applications built on top of it.
...
By making these guarantees explicit, Tenra allows teams to build systems that are both predictable and scalable without relying on implicit assumptions about how execution behaves.
