...
The Tenra API is organized around a single architectural principle:

>**Execution happens inside a runtime boundary, not through isolated function calls.**

Every major API surface in Tenra exists to support one coordinated execution model. Context establishes execution scope, models coordinate operations, providers resolve infrastructure, and MongoDB performs persistence inside a consistent runtime path.
...
<DocOverviewCards 
eyebrow="API Surface" title="Each API owns one responsibility inside the same execution model." description="Tenra organizes context, models, schemas, providers, clients, middleware, transactions, instrumentation, and events around one coordinated runtime boundary." accent="#4568f2" :signals='["Context", "Model", "Schema", "Provider", "Client", "Middleware", "Events"]'
...
At a high level, every Tenra application follows the same execution path:
...
This separation is what allows Tenra to support multi-tenancy, transaction-aware execution, runtime portability, and observability without pushing infrastructure logic into business code.
...
`TenraContext` establishes the execution boundary.
...
`TenraModel` is the primary execution surface for application-level data access.
...
`TenraSchema` defines structural and behavioral rules around persistence.
...
In most applications, this responsibility is fulfilled by `TenraClient`.

### Client

`TenraClient` is the infrastructure bridge between Tenra and MongoDB.
...
Beyond persistence access, Tenra also exposes APIs that shape execution behavior across the runtime.
...
Tenra also provides APIs responsible for runtime initialization and framework integration.

### Bootstrap

`TenraBootstrap` prepares the runtime environment.
...
Adapters connect Tenra to external runtimes such as Express, Fastify, NestJS, GraphQL servers, and serverless environments.

Their responsibility is to normalize incoming execution into Tenra’s runtime model and establish the context boundary for downstream operations.
...
The Tenra API is intentionally organized around runtime-oriented design principles.
...
The Tenra API is not a collection of isolated utilities. It is a structured interface to a coordinated execution runtime.

By separating context, execution behavior, infrastructure resolution, and persistence into clearly defined layers, Tenra allows applications to support complex runtime behavior without losing architectural coherence.

Understanding how these APIs fit together is the foundation for using Tenra effectively.
